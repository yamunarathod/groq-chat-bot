...
def main():
    ...
    client = Groq(api_key="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")
    ...
    st.write('Welcome! Feel free to ask questions about the data contained in the `hotels.csv` and `details.csv` files. You might ask about specific employee details or inquire about details records. For example, you could ask "Who are the hotels?" or "What are the most recent details?". The application matches your question to SQL queries to provide accurate and relevant results. Enjoy exploring the data!')
...
def get_reflection(client,full_prompt,llm_response,model):
    ...
    2. The query SPECIFICALLY references the correct tables: hotels.csv and details.csv, and those tables are properly aliased? (this is the most likely cause of failure)
